...
class _OrderRefundsSubState extends State<OrderRefundsSub>
  ...
  void initState() {
    ...
    for(int i=0; i<widget.data2.length; i++) {
     ...
     debugPrint('phyopyaesohn result test2 ' + widget.data.toString());
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    ...
    debugPrint('stream sub ' + widget.docId);
    return Container(
      color: Colors.white,
      child: IgnorePointer(
        ...
        child: SafeArea(
            ...
            child: Column(
                ...
                children: [
                  Top80AppBar('#1' +
                      widget.data.split('^')[1] + ' (' + (widget.data.split('^')[3].split('&')[0]=='No customer'? (widget.isEnglish? 'Walk-in customer': 'အမည်မသိ ဖောက်သည်'): widget.data.split('^')[3].split('&')[0]) + ')',
                      '$currencyUnit ' + (double.parse(widget.data.split('^')[2]).toStringAsFixed(1)).replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')),

                  // orderDateId(widget.data)
                  if (widget.docId != null && widget.docId != '')
                    StreamBuilder<SaleOrder?>(
                        stream: objectbox.getOrder(int.parse(widget.docId)),
                        builder: (BuildContext context, snapshot) {
                          if(snapshot.data!=null) {
                            ...
                            List<OrdProd> prodListObj = saleOrder.ordProds;
                            ...
                            for (int i=0; i < prodListObj.length; i++) {
                              ...
                              debugPrint('offline sub prodlist ' + prodListObj[i].uname + ' --> ' + prodListObj[i].text);
                            }
...
// width: 0.0 produces a thin "hairline" border
                                              ...
                                              if (calHourFromTZ(now).toString() != "390") {
                                                ...
                                                FirebaseFirestore.instance.collection('shops').doc(widget.shopId).collection('collArr').doc('cusArr')
                                                  ...
                                                  try{
                                                    ...
                                                    print('refNum ' + refCondition.toString());
                                                    if(refCondition) {
                                                      batch.commit();
                                                      Future.delayed(
                                                          const Duration(
  ...
  changeSaleUnit2Stock(String split) {
    if(split == 'unit_name') {
      return 'sm';
    } else {
      return 's' + (int.parse(split[3])).toString();
    }
  }

  changeBuyUnit2Stock(String split) {
    if(split == 'unit_name') {
      return 'bm';
    } else {
      return 'b' + (int.parse(split[3])).toString();
    }
  }
