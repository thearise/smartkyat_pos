...
class HomeFragmentState extends State<HomeFragment>
  ...
  fetchOrders(snapshot0, snapshot1) async {
    ...
    for(int loopOrd = 0; loopOrd < snapshot0.data!.docs.length; loopOrd++) {
      ...
      double totalDay = 0;
      // debugPrint('checkcheck1 ' + data['date'].toDate().toString().substring(0, 10));
      for(int i=0; i<=24 ; i++ ){
      ...
      if(todayModify.difference(data['date'].toDate()) < Duration(days: 31) && todayModify.difference(data['date'].toDate()) > Duration(days: 0)) {
        int i = 0;
        debugPrint('checkcheck ' + todayModify.difference(data['date'].toDate()).inDays.toString() + ' gg ' + todayModify.toString() + ' __ ' + data['date'].toDate().toString() + ' ' + todayModify.difference(data['date'].toDate()).toString());
                        ...
                        if(snapshot0.hasData) {
                          return StreamBuilder(
                              ...
                              builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot1) {
                                // debugPrint('LEN ' + snapshot0.data!.docs.length.toString());
