...
class _BlocHomeMonthState extends State<BlocHomeMonth> {
  ...
  @override
  void initState() {
    debugPrint('lengthing month 1 ' + DateFormat("yyyy-MM-dd").parse(widget.dateTime!.year.toString() + '-' + zeroToTen(widget.dateTime!.month.toString()) + '-01').toString());
    debugPrint('lengthing month 2 ' + DateFormat("yyyy-MM-dd").parse(nextYear(widget.dateTime!.month, widget.dateTime!.year).toString() + '-' + zeroToTen((nextMonth(widget.dateTime!.month)).toString()) + '-01').toString());
    prodsSnap =  FirebaseFirestore.instance.collection('shops').doc(widget.shopId).collection('collArr').doc('prodsArr').snapshots();
    prodSaleData =  FirebaseFirestore.instance.collection('shops').doc(widget.shopId).collection('prodMthData')
        .where('date', isGreaterThanOrEqualTo: DateFormat("yyyy-MM-dd").parse(widget.dateTime!.year.toString() + '-' + zeroToTen(widget.dateTime!.month.toString()) + '-01'))
        .where('date', isLessThan: DateFormat("yyyy-MM-dd").parse(nextYear(widget.dateTime!.month, widget.dateTime!.year).toString() + '-' + zeroToTen((nextMonth(widget.dateTime!.month)).toString()) + '-01'))
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        //               if(snapshot.hasData) {
                        //                 debugPrint('iphone month len ' + snapshot.data!.docs.length.toString());
                        //                 var prodsDoc;
                        //                 if(snapshot.data!.docs.length==0) {
                        //                   return Container();
                        //                 }
                        //                 prodsDoc = snapshot.data!.docs[0].data()! as Map<String, dynamic>;
                        //                 debugPrint('iphone month ' + prodsDoc['date'].toDate().toString());
                        //                 if(prodsDoc != null) {
